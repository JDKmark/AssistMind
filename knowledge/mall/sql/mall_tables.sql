/*
 mall 商城数据库表结构（节选自 document/sql/mall.sql）
 来源: https://github.com/macrozheng/mall/blob/master/document/sql/mall.sql
 说明: 仅保留 CREATE TABLE 语句（表结构部分），省略了原始文件中的
 DROP TABLE 与 INSERT INTO 数据语句。原始文件为 Navicat 导出的
 MySQL DDL（407,690 字节），共 76 张表；本节选文件为提取的
 CREATE TABLE 语句，表级注释位于语句末尾 COMMENT = 'xxx'。
*/
