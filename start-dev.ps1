...
# 1. 启动 Docker 依赖
Write-Host "[1/5] 启动 Docker 依赖服务..." -ForegroundColor Yellow
docker-compose up -d
...
# 3. 启动后端
Write-Host "[3/5] 启动后端（端口 8002）..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-Command", "cd backend; uvicorn app.main:app --reload --port 8002"

# 4. 启动 RQ worker（消费异步任务：知识库重建/坏例回流/评估运行）
Write-Host "[4/5] 启动 RQ worker（异步任务消费者）..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-Command", "cd backend; venv\Scripts\python.exe scripts/run_worker.py"

# 5. 启动前端
Write-Host "[5/5] 启动前端（端口 5173）..." -ForegroundColor Yellow
