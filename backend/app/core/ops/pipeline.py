"""运维诊断流水线：计划 → 采集 → 分析（供 Agent 编排与 SSE 流式共用）。

职责：
- `_plan`：LLM 决策采集范围（服务/数据源/关键词），失败降级全量采集
- `collect`：并行 Worker 采集证据（指标/日志/变更/告警/知识库/历史工单/拓扑）
- `analyze`：LLM 综合证据输出结构化诊断报告，失败降级规则化报告

与 `agents/ops_supervisor.py` 的关系：本模块是业务流水线（无编排状态），
Agent 层（LangGraph 节点）与 API 层（SSE 流式逐阶段 emit）都直接消费这里
的函数；Langfuse span 通过 OTEL context 自动嵌套进调用方的 trace。
...
def _parse_plan(resp: str) -> dict[str, Any]:
    ...
    if start != -1 and end > start:
        try:
            ...
            logger.warning("[OpsPipeline] 计划 JSON 解析失败: %s, raw=%r", e, raw)
    logger.warning("[OpsPipeline] 计划解析失败，将降级全量采集: %r", raw)
    return {}
...
async def _plan(query: str) -> dict[str, Any]:
    ...
    with _langfuse_span("plan", input=query) as span:
        try:
            ...
            logger.warning("[OpsPipeline] LLM 不可用，降级全量采集: %s", e)
            plan = {}
        except Exception as e:
            logger.warning("[OpsPipeline] 计划决策异常，降级全量采集: %s", e)
            plan = {}
...
async def collect(plan: dict[str, Any], query: str) -> dict[str, Any]:
    ...
    with _langfuse_span("collect", input={"query": query, "plan": plan}) as span:
        ...
        try:
            ...
            logger.warning("[OpsPipeline] 告警采集失败: %s", e)
        ...
        try:
            ...
            logger.warning("[OpsPipeline] 历史工单检索失败: %s", e)
        ...
        try:
            ...
            logger.warning("[OpsPipeline] 主机拓扑采集失败: %s", e)
        ...
        if "logs" in sources:
            try:
                ...
                logger.warning("[OpsPipeline] 日志采集失败: %s", e)
        ...
        if "changes" in sources:
            try:
                ...
                logger.warning("[OpsPipeline] 变更采集失败: %s", e)
        ...
        if "kb" in sources:
            try:
                ...
                logger.warning("[OpsPipeline] 知识库检索失败: %s", e)
                degraded.append("kb")
...
def _parse_report(resp: str) -> dict[str, Any]:
    ...
    if start != -1 and end > start:
        try:
            ...
            logger.warning("[OpsPipeline] 报告 JSON 解析失败: %s", e)
    # 兜底：从证据生成规则报告
...
async def analyze(query: str, evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    with _langfuse_span("analyze", input={"query": query, "evidence_counts": evidence_counts}) as span:
        ...
        try:
            ...
            logger.warning("[OpsPipeline] LLM 不可用，输出规则报告: %s", e)
            report = _fallback_report()
        except Exception as e:
            logger.warning("[OpsPipeline] 分析异常，输出规则报告: %s", e)
            report = _fallback_report()
