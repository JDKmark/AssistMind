...
EVAL_DATA_PATH = Path(__file__).resolve().parents[2] / "app" / "data" / "eval_mall_qa.json"


def _load_eval_data() -> list[dict]:
    """读取并解析 eval_mall_qa.json，返回条目列表（非数组时抛断言）。"""
    with EVAL_DATA_PATH.open(encoding="utf-8") as f:
        data = json.load(f)
    assert isinstance(data, list), "eval_mall_qa.json 必须是 JSON 数组"
    return data


def test_eval_data_is_json_array():
    """eval_mall_qa.json 能解析为 JSON 数组。"""
    data = _load_eval_data()
    assert isinstance(data, list)


def test_eval_data_has_enough_entries():
    """条目数 >= 10。"""
    assert len(_load_eval_data()) >= 10


def test_eval_data_each_has_nonempty_question_and_ground_truth():
    """每条含非空 question / ground_truth。"""
    for i, item in enumerate(_load_eval_data()):
